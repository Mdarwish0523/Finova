...
export const recurringExpenses = sqliteTable(
  ...
  (table) => [index("recurring_expenses_active_index").on(table.active)],
);
...
export const transactions = sqliteTable(
  ...
  (table) => [
    ...
    index("transactions_kind_date_index").on(table.kind, table.transaction_date),
...
export const settings = sqliteTable("settings", {
  ...
  starting_balance_cents: integer("starting_balance_cents").notNull().default(0),
  created_at: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
  updated_at: text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`),
});
...
export const freeTrials = sqliteTable(
  "free_trials",
  {
    ...
    remind_two_days: integer("remind_two_days", { mode: "boolean" }).notNull().default(true),
    remind_one_day: integer("remind_one_day", { mode: "boolean" }).notNull().default(true),
    created_at: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
    updated_at: text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`),
  },
  (table) => [index("free_trials_status_charge_index").on(table.status, table.charge_date)],
);
